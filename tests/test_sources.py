...
def test_bfarm_parser_and_normalization() -> None:
    html = """
    <html><body>
      <article><a href="/node/123">Example Device Recall</a><p>Published 2024-05-01 Manufacturer ABC</p></article>
    </body></html>
    """
    parsed = parse_bfarm_results(html, "https://www.bfarm.de")
    assert parsed[0]["url"] == "https://www.bfarm.de/node/123"
    normalized = normalize_bfarm_record(parsed[0])
    assert normalized["source"] == "BFARM_RECALLS"
    assert normalized["event_link"] == "https://www.bfarm.de/node/123"


def test_bfarm_direct_fetch_mock_http() -> None:
    session = FakeTgaSession([make_response(200, b"<html><body><article><a href='/node/123'>Example Device Recall</a><p>Published 2024-05-01 Manufacturer ABC</p></article></body></html>", "text/html")])
    config = BfarmClientConfig(base_url="https://www.bfarm.de", search_path="/DE/Medizinprodukte/Aufgaben/Risikobewertung-und-Forschung/Massnahmen-von-Herstellern/_node.html")
    df, warnings = fetch_bfarm_recalls_direct(["example"], [2024], client_config=config, session=session)
